...
describe('MMR Rule 1\n\tVaccineName should be Measles, Mumps, and Rubella Virus Vaccine, Patient birthdate should be > 12 mon, No previous dose, No Recommendations', () => {
  ...
  test( 'Testing age and recommendation', async () => {
    ...
    patient.birthDate = '2023-09-12';

    let imm = {
      'resourceType': 'Immunization',
      'id': 'a6d0e2b1-03d4-4929-82db-aeae27948ecd',
      'status': 'completed',
      'primarySource': true,
      'vaccineCode': {
        'coding': [
        ...
        'text': 'measles, mumps, rubella, and varicella virus vaccine',
      },
      'occurrenceDateTime': '2023-08-12T00:00:00.000Z',
    };

    const codeService = new vsac.CodeService(VALUESETS_CACHE, true, true);
    const result = await executeCql(patient, rule, libraries, { 'Imm': [imm] }, codeService, API_KEY);
    expect(result)
      .not
      .toBeNull();
    const patientResult = result[patient.id];
